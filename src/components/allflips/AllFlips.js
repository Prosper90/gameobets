...
function AllFlips() {
  ...
  const { dataUsers, userData, setUserdata, notify, dataRecent } =
    useContext(ShopContext);

  useEffect(() => {
    console.log(dataRecent, "checking from all flips component");
    if (dataRecent) {
      setUserdata(Array.from(dataRecent).reverse());
    }
  }, [address, dataRecent, notify]);
    ...
    <div className="w-100 h-[100dvh] flex justify-center items-center">
      {dataRecent?.length !== 0 ? (
        <div className="flex justify-center items-center flex-col mt-9 px-4 md:px-16">
          ...
          <div className="h-[50dvh] w-[100%] md:w-[70%] bg-[#000] border-2 border-[#FFF] to-[#0F0F0F] from-[#000] bg-gradient-to-br rounded mb-10">
            {dataRecent?.map((data) => (
        ...
        <div className="text-[#FFF] text-center font-bold rounded-lg w-[300px] h-[374px] bg-[#000] flex justify-center items-center">
          <div className="">
            <svg
              width="82"
              height="90"
            ...
            <div className="mt-5">No history</div>
          </div>
        </div>
      )}
    </div>

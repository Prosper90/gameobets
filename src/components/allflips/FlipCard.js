    ...
    return (
        <div className="border-2 border-[#FDE047]  to-[#0F0F0F] from-[#000] bg-gradient-to-br  px-5 mb-2 flex items-center relative justify-between rounded-md w-full text-white h-16">
            <div className="flex items-center md:gap-8">
                <button onClick={() => handleCopyClick(`${data.id}`)} className='cursor-pointer' title="copy">
                ...
                <h1 className='text-white game-font text-[5px]  md:text-xs'>{shortenAddress(String((Math.round(data.id/10) * 10 ) / 10**18))}</h1>
            </div>
            <div className="flex justify-between w-2/4">
                <div className="flex flex-col text-[9px] md:text-lg items-center w-1/5">
                    <b>
                    {
                       data.isWin ? ((Math.round(data.amount/10) * 10 ) / 10**18) * 2 : (Math.round(data.amount/10) * 10 ) / 10**18
                     }
                    </b>
                </div>
                ...
                <div className="flex flex-col text-[9px] md:text-lg items-center w-1/5">
                    <b>
                     {
                        parseInt(data.input)  == 1 ? "Heads" : "Tails"
                     }
                    </b>
                </div>
                <div className="flex flex-col text-[9px] md:text-lg  items-center w-1/5">

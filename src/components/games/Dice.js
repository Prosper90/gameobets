...
export default function Dice({ setSelectedGame }) {
  ...
  const handleDrag = (e, ui) => {
    ...
    // Set position within bounds (adjust 100 based on your slider width)
    const boundedPosition = Math.min(Math.max(newPosition, 0), 240);

    // Update state
    setPosition(boundedPosition);
    setnewPosition((boundedPosition / 240) * 100);
    setWinPer((boundedPosition / 240) * 95);
    setMul((boundedPosition / 240) * 1.05);
    setPayout((boundedPosition / 240) * 0.026);
  };
  ...
  //   // Set position within bounds (adjust 100 based on your slider width)
  //   const boundedPosition = Math.min(Math.max(newPosition, 0), 240);
  //   //relationship between 240 and 100 is 140
  //   // Update state
  //   setPosition(boundedPosition);
  //   setnewPosition((boundedPosition / 240) * 100);
  // };
  ...
  const roll = () => {
    ...
    setSelectedChoice(newPosition.toFixed());
    setWinchance(winper);
    play(searchParams);
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center rounded-2xl bg-[#130D25] w-[100%] md:w-[28%] justify-start scroll-m-0 overflow-y-scroll scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none'] text-white text-opacity-60">
        ...
        <h1 className="game-font text-[#FFF] text-lg font-bold leading-[15px] mb-[20px]">
          <div className="text-center text-2xl">{newPosition.toFixed(2)}</div>
          Roll under
        </h1>

        <div className="flex justify-between text-[#fff] font-extrabold text-xs border border-[#2a253a] w-[80%] p-[6px] mb-[10px] rounded">
          <div className="text-center">
            <div className="">{winper.toFixed(0)}%</div>
          ...
          <div className="text-center">
            <div className="">{payout.toFixed(2)}ETH</div>
        ...
        <div className="flex flex-col gap-2 text-sm ">
          ...
          <div className="flex justify-between">
            <div className="p-1 border border-[#2a253a] rounded">0</div>
            ...
            <div className="p-1 border border-[#2a253a] rounded">100</div>
          </div>
        </div>
        ...
        <div className="flex justify-center mt-5">
          <div className="px-3 mb-6 md:mb-5 flex flex-col justify-center items-center">
            <label className="border-2 border-[#1cba6b] rounded pl-5 py-5 h-10 flex justify-center items-center">
              ...
              />
              <div onClick={roll} className="w-[100px] text-[#fff] border-l-2 border-[#1cba6b] font-extrabold p-3 game-font text-sm md:text-sm cursor-pointer text-center hover:bg-[#1cba6b] hover:text-[#fff]">
                Roll
              </div>
            </label>
          </div>
